...
`direction` | Direction of the rate | `string('inbound', 'outbound', 'both')` | `both` | `false`
...
> GET /v2/rates/reseller

```shell
curl -v -X GET \
    -H "X-Auth-Token: {AUTH_TOKEN}" \
    http://{SERVER}:8000/v2/rates/reseller
```

#### Change

> POST /v2/rates/reseller
...
> DELETE /v2/rates/reseller/{RATE_ID}

```shell
curl -v -X DELETE \
    -H "X-Auth-Token: {AUTH_TOKEN}" \
    http://{SERVER}:8000/v2/rates/reseller/{RATE_ID}
```

#### Fetch

> GET /v2/rates/reseller/{RATE_ID}
...
> PATCH /v2/rates/reseller/{RATE_ID}
...
> POST /v2/rates/reseller/{RATE_ID}
...
> GET /v2/rates/number/{NUM}/{DIRECTION}

```shell
curl -v -X GET \
    -H "X-Auth-Token: {AUTH_TOKEN}" \
    http://{SERVER}:8000/v2/rates/number/{NUM}/{DIRECTION}
```
