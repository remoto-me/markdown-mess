...
Similar to the PUT, POST will update an existing config:
    ...
    POST /v2/accounts/{ACCOUNT\_ID}/notifications/{NOTIFICATION\_ID}
...
Remove a notification template
    ...
    DELETE /v2/accounts/{ACCOUNT\_ID}/notifications/{NOTIFICATION\_ID}

Omit the ``/accounts/{ACCOUNT_ID}`` to remove the system default.

    DELETE /v2/notifications/{NOTIFICATION\_ID}
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    GET /v2/accounts/{ACCOUNT\_ID}/notifications/smtplog
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    GET /v2/accounts/{ACCOUNT\_ID}/notifications/smtplog/{SMTP\_LOG\_ID}
